module.exports = function(message, commandName, args, client) {
    if (message.channel.nsfw === true || message.guild === null) {
        if (args[0] === "gif") {
            client.request(`https://api.nekos.dev/api/v3/images/nsfw/gif/pussy`).then(res => res.text()).then(body => {
                if (body.trim() !== "") {
            ...
            client.request(`https://api.nekos.dev/api/v3/images/nsfw/img/pussy_lewd`).then(res => res.text()).then(body => {
                if (body.trim() !== "") {
                    var jsonParsed = JSON.parse(body);
                    const Embed = {
                        color: '#00ff00',
