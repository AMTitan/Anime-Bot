module.exports = function(message, commandName, args, client) {
    if (message.channel.nsfw === true || message.guild === null) {
        client.request(`https://api.nekos.dev/api/v3/images/nsfw/img/pantyhose_lewd/`).then(res => res.text()).then(body => {
            if (body.trim() !== "") {
                var jsonParsed = JSON.parse(body);
                const Embed = {
                    color: '#00ff00',
                    title: 'Pantyhose nsfw',
                    url: "",
                    author: {
                        Name: 'AnimeBot',
...
module.exports.config = {
    description: "Gives you a pantyhose image",
    usage: `pantyhose`,
    aliases: [],
    type: "nsfw",
    optinal: ""
