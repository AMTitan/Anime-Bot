		...
		.then(collected => {
			...
			if (nsfw == true) {
				if (message.channel.nsfw === true || message.guild === null) {
					request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=${types[3]} ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
				...
				if (message.channel.nsfw === true || message.guild === null) {
					request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=${types[2]} ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
						if (!error && response.statusCode == 200) {
							var shouldIReturn = false;
							if (!body) {
								const Embed = {
				...
				request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=${types[1]} ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
					if (!error && response.statusCode == 200) {
						var shouldIReturn = false;
						if (!body) {
							const Embed = {
