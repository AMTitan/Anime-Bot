		...
		.then(collected => {
			...
			if (nsfw == true) {
				if (message.channel.nsfw === true || message.guild === null) {
					request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=-loli -asian -3d ${types[3]} ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
				...
				request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=-loli -asian -3d ${types[1]} ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
					if (!error && response.statusCode == 200) {
						var shouldIReturn = false;
						if (!body) {
							const Embed = {
				...
				if (message.channel.nsfw === true || message.guild === null) {
					request(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tags=-loli -asian -3d ${commandName + " " + args.join(" ")}&json=1`, function(error, response, body) {
						if (!error && response.statusCode == 200) {
							var shouldIReturn = false;
							if (!body) {
								const Embed = {
