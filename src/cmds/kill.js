module.exports = function(Prefix, message, commandName, args, request, client) {
    if (message.channel.nsfw === true || message.guild === null) {
        request(`https://api.waifu.pics/sfw/kill`, function(error, response, body) {
            if (!error && response.statusCode == 200) {
                var jsonParsed = JSON.parse(body);
                var title = "kill";
                if (message.mentions.users && message.mentions.users.first()) title = `${message.author.username} killed ${message.mentions.users.first().username}`;
                const Embed = {
                    color: '#00ff00',
                    title: title,
                    url: "",
